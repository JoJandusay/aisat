<x-layouts.app>
    ...
    <div class="pb-20">
        ...
        <div class="bg-white w-full shadow-md rounded-lg p-4">
            <div id="chart"></div>
        </div>

        <div class="w-full overflow-hidden bg-white shadow-md rounded-lg p-4 py-6 mt-4">
            <div>
        ...
        <div class="overflow-hidden bg-white w-full shadow-md rounded-lg p-4 py-6 mt-4">
            <div>
                        ...
                        @forelse ($clinic_students as $clinic_student)
                            <tr class="text-gray-700 dark:text-gray-400 divide-x">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <span class="text-xs">
                                        ({{ $clinic_student->student->level }}
                                        {{ $clinic_student->student->section }})
                                    </span>
                                </td>
                                <td class="px-4 py-3">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            let reports = @json($reports_this_month)

            let reportDates = reports.map(report => report.report_date);
            let reportCounts = reports.map(report => report.total);

            var options = {
                series: [{
                    name: "Reported Students",
                    data: reportCounts
                }],
                chart: {
                    type: 'area',
                ...
                title: {
                    text: 'Number of Reported Students this Month',
                    align: 'left'
                },
                labels: reportDates, // Use extracted dates
                xaxis: {
                    type: 'category', // Use 'category' if dates are strings
